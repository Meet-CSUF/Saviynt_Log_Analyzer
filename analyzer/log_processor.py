...
class LogProcessor:
    ...
    def process_next_file(self):
        ...
        try:
            ...
            logger.info(f"Processed file: {file_path}, lines processed: {lines_processed_in_file}, lines skipped: {skipped_lines}")
            return True
    ...
    def get_logs_by_class_and_level(self, class_name, level, page=1, page_size=100, search_query=None, use_regex=False):
        """Fetch logs for a specific class and level on-demand with pagination."""
        try:
                        ...
                        for i, line in enumerate(f):
                            if i == line_idx:
                                try:
                                    ...
                                    # Validate class matches requested class_name
                                    ...
                                    if search_query:
                                        try:
                                            if not (use_regex and (
                                                ...
                                                total_logs -= 1  # Adjust total if filtered out
                                                break
                                        except re.error:
    ...
    def get_logs_by_service_and_level(self, service_name, level, page=1, page_size=100, search_query=None, use_regex=False):
        """Fetch logs for a specific service and level on-demand with pagination."""
        try:
